...
const PokemonForm: React.FC<Props> = props => {
    ...
    async function savePokemonHandler(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        ...
        if (pokemon.name.length === 0) {
            ...
            if (isEditing) {
                try {
                    await fetch(`https://devsu-pokecrud.herokuapp.com/api/pokemons/${isEditing.id}`, {
                        method: 'PUT',
                ...
                try {
                    const response = await fetch('https://devsu-pokecrud.herokuapp.com/api/pokemons/', {
                        method: 'POST',
                        body: JSON.stringify(pokemon),
                        headers: {
                            'Content-Type': 'application/json',
                    ...
                    await fetch(`https://devsu-pokecrud.herokuapp.com/api/pokemons/${pokemon.id}`, {
                        method: 'PUT',
                        body: JSON.stringify(pokemon),
                        headers: {
                            'Content-Type': 'application/json',
